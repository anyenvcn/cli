...
export function daemonPaths() {
  ...
  const statePath = process.env.ANYENV_DAEMON_STATE
    ? path.resolve(process.env.ANYENV_DAEMON_STATE)
    : path.join(configDir, "daemon.json");
  const logPath = process.env.ANYENV_DAEMON_LOG
    ? path.resolve(process.env.ANYENV_DAEMON_LOG)
    : path.join(configDir, "daemon.log");
...
export function patchDaemonState(patch = {}) {
...
export function daemonStatus() {
  ...
  const lastEventAt = state?.lastEventAt || state?.lastHeartbeatAt || state?.startedAt || "";
  const lastEventTime = lastEventAt ? Date.parse(lastEventAt) : NaN;
  const staleForSeconds = state && !running && Number.isFinite(lastEventTime)
    ? Math.max(0, Math.round((Date.now() - lastEventTime) / 1000))
    : null;
  return {
    ok: true,
    running,
