...
import { startEmbeddedVncServer } from "./embedded-vnc.js";
...
function remoteDesktopSetupInstructions() {
  return {
    macos: [
      "运行 anyenv restart --allow-remote-desktop；CLI 会启动内置远程桌面 helper，不要求额外安装 VNC Server。",
      "首次接管真实系统桌面时，macOS 仍可能要求屏幕录制或辅助功能权限。",
    ],
    linux: [
      "运行 anyenv restart --allow-remote-desktop；CLI 会启动内置远程桌面 helper，headless 环境也能显示 AnyEnv CLI Desktop。",
      "需要接入已有系统 VNC 时，再使用 --vnc-port 指定真实端口。",
    ],
    windows: [
      "运行 anyenv restart --allow-remote-desktop；CLI 会启动内置远程桌面 helper，不要求额外安装 VNC Server。",
      "首次接管真实系统桌面时，Windows 仍可能要求桌面捕获或输入控制权限。",
    ],
  };
}
...
function remoteDesktopPolicy(options = {}) {
  ...
  const helperMode = remoteDesktopHelperMode(options);
  const embeddedDefault = portInfo.portMode === "auto" && helperMode !== "external";
  return {
    enabled: Boolean(options.allowRemoteDesktop),
    mode: embeddedDefault ? "cli-managed-rfb" : "experimental-local-vnc",
    protocol: "vnc",
    host: "127.0.0.1",
    port: portInfo.port,
    portMode: portInfo.portMode,
    helperMode,
    source: embeddedDefault ? "cli-embedded" : "external-vnc",
    candidatePorts: portInfo.candidatePorts,
    setup: remoteDesktopSetupInstructions(),
  };
}

function remoteDesktopHelperMode(options = {}) {
  const raw = String(options.remoteDesktopHelper || process.env.ANYENV_REMOTE_DESKTOP_HELPER || "auto").trim().toLowerCase();
  if (raw === "external" || raw === "external-vnc") return "external";
  if (raw === "embedded" || raw === "managed" || raw === "cli") return "embedded";
  return "auto";
}
...
function runVncCleanup(cleanup, log, requestId) {
  if (typeof cleanup !== "function") return;
  try {
    const result = cleanup();
    if (result && typeof result.then === "function") {
      result.catch((error) => log("vnc.cleanup.failed", { requestId, message: errorMessage(error) }));
    }
  } catch (error) {
    log("vnc.cleanup.failed", { requestId, message: errorMessage(error) });
  }
}
...
function connectLocalVncCandidate(requestId, policy, port, sessions, sendFrame, log, handshakeTimeoutMs, cleanup = null) {
  ...
  return new Promise((resolve) => {
    ...
    const cleanupOnce = () => {
      if (cleaned) return;
      cleaned = true;
      runVncCleanup(cleanup, log, requestId);
    };
    ...
    tcp.on("data", (chunk) => {
      ...
      if (!opened) {
        ...
        clearHandshakeTimer();
        opened = true;
        sessions.set(requestId, { socket: tcp, policy: candidatePolicy, cleanup: cleanupOnce });
        log("vnc.open.connected", { requestId, host: candidatePolicy.host, port, portMode: candidatePolicy.portMode, source: candidatePolicy.source || "", handshake: "rfb" });
        settle({
          type: "vnc.open.response",
          requestId,
          ok: true,
...
async function openLocalVncSession(message = {}, options = {}, sessions, sendFrame, log = () => {}) {
  ...
  const attempts = [];
  if (policy.portMode === "auto" && policy.helperMode !== "external") {
    let embedded = null;
    try {
      embedded = await startEmbeddedVncServer();
      const embeddedPolicy = {
        ...policy,
        mode: embedded.mode,
        source: embedded.source,
        port: embedded.port,
        resolvedPort: embedded.port,
      };
      log("vnc.embedded.started", { requestId, host: embedded.host, port: embedded.port });
      const response = await connectLocalVncCandidate(
        ...
        sendFrame,
        log,
        vncHandshakeTimeoutMs(),
        () => embedded.close(),
      );
      if (response.ok) return response;
      attempts.push({
        port: embedded.port,
        code: response.code || "embedded_vnc_failed",
        detail: response.detail || response.error || "",
      });
      if (policy.helperMode === "embedded") return response;
    } catch (error) {
      if (embedded) runVncCleanup(() => embedded.close(), log, requestId);
      const message = errorMessage(error);
      log("vnc.embedded.failed", { requestId, message });
      attempts.push({
        port: 0,
        code: "embedded_vnc_start_failed",
        detail: message,
      });
      if (policy.helperMode === "embedded") {
        return {
          type: "vnc.open.response",
          requestId,
          ok: false,
          code: "embedded_vnc_start_failed",
          error: `CLI 内置远程桌面 helper 启动失败: ${message}`,
          remoteDesktop: {
            ...policy,
            resolvedPort: null,
          },
  ...
  return {
    ...
    error: `CLI 内置远程桌面 helper 和本机候选 VNC/RFB 端口均不可用。已探测: ${candidatePorts.join(", ")}。可设置 ANYENV_REMOTE_DESKTOP_HELPER=external 只连接外部 VNC，或使用 --vnc-port 指定真实端口。`,
    detail,
    remoteDesktop: {
      ...policy,
      resolvedPort: null,
...
function closeLocalVncSessions(sessions, reason = "websocket_closed") {
  for (const [requestId, session] of sessions.entries()) {
    ...
    runVncCleanup(session.cleanup, () => {}, requestId);
  }
}
