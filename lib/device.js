...
function deviceLog(event, detail = {}) {
  ...
  const line = `[AnyEnv:device] ${new Date().toISOString()} ${event}${suffix}\n`;
...
function remoteDesktopSetupInstructions() {
  return {
    macos: [
      ...
      "然后重新启动 CLI：anyenv restart --workspace . --allow-remote-desktop",
    ],
    linux: [
...
function openLocalVncSession(message = {}, options = {}, sessions, sendFrame, log = () => {}) {
  ...
  if (!policy.enabled) {
    return Promise.resolve({
      ...
      error: "本机远程桌面能力未开启。请在本机运行 anyenv restart --workspace . --allow-remote-desktop 后重试。",
      remoteDesktop: policy,
    });
  }
  ...
  return new Promise((resolve) => {
    ...
    tcp.on("connect", () => {
      ...
      log("vnc.open.connected", { requestId, host: policy.host, port: policy.port });
      settle({
        type: "vnc.open.response",
        requestId,
        ok: true,
