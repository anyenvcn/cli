...
function remoteDesktopSetupInstructions() {
  return {
    macos: [
      "打开系统设置 -> 通用 -> 共享 -> 屏幕共享，并确认 VNC 监听在本机 5900 端口。",
      "然后重新启动 CLI：anyenv restart --allow-remote-desktop",
    ],
    linux: [
      "安装 x11vnc、tigervnc 或 wayvnc，并让 VNC 服务监听 127.0.0.1:5900。",
      "然后重新启动 CLI：anyenv restart --allow-remote-desktop",
    ],
    windows: [
...
function remoteDesktopPolicy(options = {}) {
  return {
    ...
    protocol: "vnc",
    host: "127.0.0.1",
    port: clampNumber(options.vncPort, 5900, 1, 65535),
...
function openLocalVncSession(message = {}, options = {}, sessions, sendFrame, log = () => {}) {
  const requestId = String(message.requestId || "");
  ...
  return new Promise((resolve) => {
    ...
    const tcp = net.createConnection({ host: policy.host, port: policy.port });
    ...
    tcp.on("connect", () => {
      ...
      handshakeTimer = setTimeout(() => {
        log("vnc.handshake.timeout", { requestId, host: policy.host, port: policy.port });
        settle({
          type: "vnc.open.response",
          requestId,
          ok: false,
          code: "vnc_handshake_timeout",
          error: `本机 ${policy.host}:${policy.port} 接受了 TCP 连接，但没有返回 VNC/RFB 握手。请确认该端口是真正的 VNC 服务，未被 Docker、Registry、HTTP 等其他服务占用。`,
          detail: `未在 ${vncHandshakeTimeoutMs()}ms 内收到 RFB 握手。`,
          remoteDesktop: policy,
        });
        tcp.destroy();
    ...
    tcp.on("data", (chunk) => {
      ...
      if (!opened) {
        ...
        if (!isRfbHandshake(handshakeBuffer)) {
          ...
          log("vnc.handshake.invalid", { requestId, host: policy.host, port: policy.port, asciiHead });
          settle({
            type: "vnc.open.response",
            requestId,
            ok: false,
            code: "vnc_handshake_invalid",
            error: `本机 ${policy.host}:${policy.port} 不是 VNC/RFB 服务。请检查 --vnc-port 是否指向真实 VNC Server。`,
        ...
        clearHandshakeTimer();
        opened = true;
        sessions.set(requestId, { socket: tcp, policy });
        log("vnc.open.connected", { requestId, host: policy.host, port: policy.port, handshake: "rfb" });
        settle({
          type: "vnc.open.response",
          requestId,
          ok: true,
    ...
    tcp.on("error", (error) => {
      ...
      if (!opened) {
        settle({
          ...
          error: `无法连接本机 VNC 服务 ${policy.host}:${policy.port}。请先安装/启动屏幕共享或 VNC Server。`,
          detail: messageText,
          remoteDesktop: policy,
        });
        return;
    ...
    tcp.on("close", () => {
      ...
      if (!opened) {
        settle({
          ...
          error: `无法连接本机 VNC 服务 ${policy.host}:${policy.port}。`,
          remoteDesktop: policy,
        });
        return;
      }
